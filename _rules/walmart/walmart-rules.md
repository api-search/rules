...
categories:
- walmart
description: Spectral linting rules defining API design standards and conventions for Walmart.
layout: rules
...
- description: All operations must have an operationId
  given: $.paths[*][*]
  name: walmart-operation-ids-required
  severity: error
- description: Operation summaries must use Title Case
  given: $.paths[*][*].summary
  name: walmart-operation-summary-title-case
  severity: warn
- description: All operations must have at least one tag
  given: $.paths[*][*]
  name: walmart-operation-tags-required
  severity: error
- description: Walmart APIs require WM_SEC.ACCESS_TOKEN, WM_QOS.CORRELATION_ID, and WM_SVC.NAME headers on all operations
  given: $.paths[*][get,post,put,patch,delete].parameters
  name: walmart-required-headers-defined
  severity: warn
- description: Walmart APIs should define both production and sandbox server URLs
  given: $.servers
  name: walmart-servers-include-sandbox
  severity: warn
- description: Info object must have a description
  given: $.info
  name: walmart-info-description
  severity: error
- description: Operations must define a successful (2xx) response
  given: $.paths[*][*].responses
  name: walmart-response-success-defined
  severity: error
- description: All paths must be versioned with /v3/ prefix for marketplace APIs
  given: $.paths
  name: walmart-paths-versioned
  severity: warn
- description: Request bodies should use application/json content type
  given: $.paths[*][post,put,patch].requestBody.content
  name: walmart-json-content-type
  severity: warn
- description: Successful responses should reference a schema component
  given: $.paths[*][*].responses['200'].content['application/json']
  name: walmart-response-schema-reference
  severity: warn
rules_file: rules/walmart-rules.yml
rules_url: https://raw.githubusercontent.com/api-evangelist/walmart/refs/heads/main/rules/walmart-rules.yml
severity_counts:
  error: 4
  hint: 0
  info: 0
  warn: 6
slug: walmart-rules
source_filename: walmart-rules.yml
...
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/walmart/refs/heads/main/rules/walmart-rules.yml
tags:
- Commerce
- Retail
- Spectral
